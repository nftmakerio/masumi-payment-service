...
export async function getWalletBalance(token: string, options: GetWalletOptions): Promise<GetWalletResponse> {
    ...
    try {
        ...
        const nmkrPolicyId = "5dac8536653edc12f6f5e1045d8164b9f59998d3bdc300fc92843489";
        ...
        const balanceNmkr = data.amount.reduce((total: number, asset: BlockfrostAsset) => {
            if (asset.unit?.startsWith(nmkrPolicyId + nmkrHex)) {
                return total + parseInt(asset.quantity || '0');
            }
            return total;
        }, 0);
        ...
        return {
            ...
            usdm: balanceUsdm / 1000000,
            nmkr: balanceNmkr / 1000000,
            status: 'success'
        };
    } catch (error) {
